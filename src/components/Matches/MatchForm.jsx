    ...
    <div className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${
      ...
      {onRemove && (
        <button
          type="button"
          onClick={onRemove}
          className="text-gray-600 hover:text-red-400 text-xl leading-none flex-shrink-0 transition-colors"
        >
          ×
        </button>
      )}
...
function PlayerSearch({ selected, onSelect, onClear, excludeIds, placeholder }) {
  ...
  async function search(q) {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('id, full_name, username, rating, league')
  ...
  if (selected) {
    return <PlayerCard player={selected} onRemove={handleClear} />
  }
  ...
  return (
    <div className="relative">
      <input
        ...
        placeholder={placeholder || 'Търсене по ime или потребителско...'}
        autoComplete="off"
      />
      {showDropdown && results.length > 0 && (
        <div
          ref={dropdownRef}
          className="absolute z-50 w-full mt-1 bg-[#1e1e1e] border border-[#2a2a2a] rounded-lg overflow-hidden shadow-xl"
        >
          {results.map(p => (
            <button
              key={p.id}
...
export default function MatchForm({ onSubmitted }) {
  ...
  // Team 1: player1 (self, auto) + player2 (partner)
  const [partner, setPartner] = useState(null)

  // Team 2: player3 (opponent1) + player4 (opponent2)
  ...
  function validateSets() {
    ...
    if (validSets.length < 2) return 'Въведете поне 2 сета.'
    for (const s of validSets) {
      const p1 = parseInt(s.p1)
      const p2 = parseInt(s.p2)
      if (isNaN(p1) || isNaN(p2) || p1 < 0 || p2 < 0) return 'Невалидни резултати.'
      if (p1 === p2) return 'Сетовете не могат да завършат равни.'
    }
    return null
  }
  ...
  async function handleSubmit(e) {
    ...
    if (!allPlayersSelected) { setError('Изберете всички 4 играча.'); return }
    const setsErr = validateSets()
    if (setsErr) { setError(setsErr); return }

    const winner = determineWinner()
    if (!winner) { setError('Не може да се определи победител от въведените резултати.'); return }
    ...
    try {
      ...
      // winner_id = team captain (player1 if team1 wins, player3/opponent1 if team2 wins)
      const winnerId = winner === 'team1' ? profile.id : opponent1.id

      const { error: insertError } = await supabase.from('matches').insert({
        player1_id: profile.id,
        player2_id: partner.id,
        player3_id: opponent1.id,
        player4_id: opponent2.id,
        winner_id: winnerId,
        match_type: matchType,
        sets_data: setsData,
        player1_rating_before: profile.rating,
        player2_rating_before: partner.rating,
        player3_rating_before: opponent1.rating,
        player4_rating_before: opponent2.rating,
        status: 'pending',
      ...
      setError(err.message || 'Грешка при записване на мача.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  // IDs already chosen — excluded from searches
  const usedIds = [
    profile?.id,
    partner?.id,
    opponent1?.id,
  ...
  return (
    <form onSubmit={handleSubmit} className="card space-y-6">
      <h2 className="text-lg font-bold text-white">Запиши нов мач</h2>

      {success && (
        <div className="p-3 bg-[#CCFF00]/10 border border-[#CCFF00]/30 rounded-lg text-[#CCFF00] text-sm font-medium">
          ✓ Мачът е записан успешно! Изчаква одобрение от администратор.
        </div>
      )}
      {error && (
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Твоят отбор</span>
        </div>

        <div>
          <p className="text-xs text-gray-500 mb-1.5">Играч 1 (Ти)</p>
          {profile && <PlayerCard player={profile} dimmed />}
        </div>

        <div>
          <p className="text-xs text-gray-500 mb-1.5">Партньор *</p>
          <PlayerSearch
            selected={partner}
            onSelect={setPartner}
            onClear={() => setPartner(null)}
            excludeIds={usedIds.filter(id => id !== partner?.id)}
            placeholder="Търсене на партньор по ime или username..."
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Противников отбор</span>
        </div>

        <div>
          <p className="text-xs text-gray-500 mb-1.5">Противник 1 *</p>
          <PlayerSearch
            selected={opponent1}
            onSelect={setOpponent1}
            onClear={() => setOpponent1(null)}
            excludeIds={usedIds.filter(id => id !== opponent1?.id)}
            placeholder="Търсене по ime или username..."
          />
        </div>

        <div>
          <p className="text-xs text-gray-500 mb-1.5">Противник 2 *</p>
          <PlayerSearch
            selected={opponent2}
      ...
      {/* ── Match type ── */}
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1.5">Формат *</label>
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold border transition-colors ${
                matchType === type
                  ...
                  : 'bg-[#111111] text-gray-400 border-[#2a2a2a] hover:border-[#CCFF00]/50'
              }`}
            >
      ...
      {/* ── Sets (locked until all 4 players selected) ── */}
      <div>
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          <label className="text-sm font-medium text-gray-300">Резултати по сетове *</label>
          {!allPlayersSelected ? (
            <span className="text-xs text-gray-600 italic">— изберете всички 4 играча първо</span>
          ) : winnerPreview ? (
            <span className={`text-sm font-semibold ${winnerPreview === 'team1' ? 'text-[#CCFF00]' : 'text-red-400'}`}>
              → Победител: {winnerPreview === 'team1' ? 'Твоят отбор' : 'Противников отбор'}
            </span>
          ) : null}
        </div>
        ...
        <div className={`space-y-2 transition-opacity ${!allPlayersSelected ? 'opacity-30 pointer-events-none select-none' : ''}`}>
          ...
          <div className="flex items-center gap-3 text-xs mb-1">
            ...
            <span className="w-16 text-center text-[#CCFF00] font-medium">Твоят</span>
            <span className="w-5 text-center text-gray-600">:</span>
            <span className="w-16 text-center text-red-400 font-medium">Против.</span>
          </div>
          ...
          {sets.map((set, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center gap-3">
                <span className="text-gray-500 text-sm w-16 flex-shrink-0">Сет {idx + 1}</span>
                <input
                  type="number"
                  value={set.p1}
                  onChange={e => updateSet(idx, 'p1', e.target.value)}
                  className="input-dark text-center w-16"
                  placeholder="0"
                  min="0"
                  max="99"
      ...
      {/* ── Club ── */}
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1.5">Клуб (място на игра)</label>
      ...
      {/* ── Date ── */}
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1.5">Дата на мача *</label>
        <input
          type="date"
          value={playedAt}
      ...
      {/* ── Notes ── */}
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1.5">Бележки (незадължително)</label>
        <textarea
          value={notes}
          onChange={e => setNotes(e.target.value)}
          className="input-dark resize-none"
          rows={2}
          placeholder="Допълнителна информация за мача..."
        />
      </div>

      <button
        type="submit"
        disabled={submitting || !allPlayersSelected}
